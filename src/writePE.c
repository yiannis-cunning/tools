...
uint8_t cmploop[57] = {0x48, 0x83, 0xec, 0x08, 0x48, 0x8d, 0x05, 0xf5, 0x1f, 0x00, 0x00, 0x48, 0x89, 0xc1, 0x48, 0x8b, 0x05, 0xeb, 0x0f, 0x00, 0x00, 0x48, 0x81, 0xf8, 0x6c, 0x20, 0x00, 0x00, 0x74, 0x18, 0x48, 0x31, 0xc0, 0x48, 0x81, 0xd0, 0x08, 0x00, 0x00, 0x00, 0x48, 0x81, 0xe8, 0x08, 0x00, 0x00, 0x00, 0x48, 0x39, 0xc0, 0x74, 0xea, 0x48, 0x83, 0xc4, 0x08, 0xc3};
...
int main(int argc, char *argv[]){
    ...
    */
    ...
    shared_header.minor_linker_version      = 26;                    //
    ...
    pe_plus_header.subsystem                = 10;                    // 10 for efi, 3 for normal windows?
	...
	pe_plus_header.stack_reserve_size       = 0;             // MAX amount of stack size that could be allocated
	pe_plus_header.stack_commit_size        = 0;               // Starting amount (1 page)
	pe_plus_header.heap_reserve_size        = 0;             // MAX reservable for heap
	pe_plus_header.heap_commit_size         = 0;               // starting allocated for heap
    ...
    pe_plus_header.import_table_rva        = 0; //code_start_rva + align(section_alignment, 50);
    pe_plus_header.import_table_size       = 0;
    pe_plus_header.iat_rva                 = 0; //pe_plus_header.import_table_rva + sizeof(import_directory_entry_t)*2 + sizeof(uint64_t)*2;
    pe_plus_header.iat_size                = 0;
    ...
    // Code headers
    strcpy(sh.name, ".text");
    sh.virtual_size                        = 57;                           // Size after being loaded into memory
    sh.virtual_address                     = code_start_rva;               // RVA of start of section
    sh.size_of_raw_data                    = align(file_alignment, 57);    // Size of raw code aligned to file aligment
    sh.pointer_to_raw_data                 = code_start_file;              // file pointer to start of this section
    sh.characteristics                     = 0x60000060;                   // Characteristic of section
